...
import { getCities } from "@/actions/listings";
...
interface CitySelectProps {
  value: string;
  onChange: (value: string, cityId: number) => void;
  error?: string;
}
...
export default function CitySelect({ value, onChange, error }: CitySelectProps) {
  ...
  return (
    <div ref={wrapperRef} className="relative">
      <label className="block text-[12px] font-light text-white/50 mb-2">İl</label>
      ...
      >
        <span className={value ? "text-white text-[14px]" : "text-white/25 text-[14px]"}>
          {loading ? "Yükleniyor..." : value || "İl Seçiniz"}
        </span>
